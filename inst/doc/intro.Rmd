...
__SC19056__ is a simple R package consisting of two functions, _LRL_ and _EMg_ . The function _LRL_ is used to solve the linear regression for matrix responses with low-rank limit. And the function _EMg_ is used to solve the EM estimation of binary mixed Gaussian model.

## Benchmarking _LRL_ 

The source R code for _LRL_ is as follows:
```{r}
...
Here, we give a example with the data _xy_ in R package __SC19056__. 
...
We use the function _image_B_ to show the real parameter _B_ and the LRL estimation _B_hat_.
...
The source R code for _EMg_ is as follows:
```{r}
...
About the _EMg_ ,dataset _faithful_ is a example.
```{r}
library(mvtnorm)
e1<-list()
